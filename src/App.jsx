...
p
          ...
          <div className="pricechart">
            <BarChart width={150} height={228} margin={{ top: 15, bottom: 15 }} data={[{ uv: this.state.egeWeeks * this.state.lessonPrice * this.state.subjectCount * this.state.lessonCount, pv: examerPrices[this.state.subjectCount - 1] }]}>
              <Bar dataKey="uv" fill="#8884d8" label={{ position: 'top', fontSize: '10' }} />
              <Bar dataKey="pv" fill="#8884d8" label={{ position: 'top', fontSize: '10' }} />
            </BarChart>
...
Экзамер в
          <b>
            {this.doRound((this.state.egeWeeks * this.state.lessonPrice * this.state.subjectCount * this.state.lessonCount) / examerPrices[this.state.subjectCount - 1])}
            {' '}
раз дешевле
          </b>
          {' '}
занятий с репетитором и сэкономит тебе
          <b>
            {' '}
            {(this.state.egeWeeks * this.state.lessonPrice * this.state.subjectCount * this.state.lessonCount) - examerPrices[this.state.subjectCount - 1]}
            {' '}
рублей
          </b>
        </h2>
